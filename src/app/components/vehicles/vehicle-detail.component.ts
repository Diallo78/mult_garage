...
@Component({
  ...
  template: `
    ...
    <div *ngIf="!isLoading" class="p-3 sm:p-4">
      <div class="space-y-4 md:space-y-6" *ngIf="vehicle">
        <!-- Header Section -->
        <div
        ...
        <!-- Owner Information -->
        <div class="card p-3 sm:p-4" *ngIf="owner">
          <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-3">
            Propriétaire
          </h3>
        ...
        <div class="card p-3 sm:p-4">
          ...
          <div
            *ngIf="serviceHistory.length === 0"
            class="text-center py-6 text-gray-500 text-sm"
          >
            Aucun historique d'entretien disponible
          </div>

          <div *ngIf="serviceHistory.length > 0" class="space-y-3">
            <div
              *ngFor="let visit of serviceHistory"
              ...
              >
                <div>
                  <h4 class="font-medium text-gray-900 text-sm sm:text-base">
                    {{ visit.visitDate | firestoreDate | date : 'mediumDate' }}
                  </h4>
                  <p class="text-xs sm:text-sm text-gray-500 line-clamp-1">
                    {{ visit.reportedIssues.join(', ') }}
                  </p>
                </div>
                <span
                  class="status-badge text-xs sm:text-sm mt-1 sm:mt-0"
...
export class VehicleDetailComponent implements OnInit {
  ...
  private async loadVehicleData(): Promise<void> {
    ...
    try {
      // Load vehicle
      this.vehicle = await this.garageDataService.getById<Vehicle>(
        'vehicles',
        this.vehicleId!
      );
      ...
      if (this.vehicle) {
        // Load owner
        this.owner = await this.garageDataService.getById<Client>(
          'clients',
          this.vehicle.clientId
        );

        // Load service history
        this.serviceHistory = await this.garageDataService.getWithFilter<Visit>(
          'visits',
