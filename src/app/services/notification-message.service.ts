...
export class NotificationMessageService {
  ...
  async getAllLatestNotificationsv1(
    emailClient: string,
    garageId: string,
    count = 100
  ): Promise<NotificationModel[]> {
    ...
    if (emailClient)
      conditions.push(where('emailDesitnateur', '==', emailClient));
  ...
  async getAllLatestNotifications(
    userEmail: string,
    garageId: string,
    role: 'Client' | 'Garage',
    count = 100
  ...
  // notification-message.service.ts
  getLiveNotifications(
    ...
    const q = query(
      collection(db, 'notifications'),
      where(
        role === 'Client' ? 'emailClient' : 'garageId',
        '==',
        role === 'Client' ? emailClient : garageId
      ),
      orderBy('createdAt', 'desc'),
