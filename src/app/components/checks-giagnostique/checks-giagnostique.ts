  ...
  <div *ngIf="!isLoading">
        ...
        <div *ngIf="categories" class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-100">
              <tr>
               <th class="px-6 py-3 text-left text-sm font-medium text-gray-700">Date création</th>
                <th class="px-6 py-3 text-left text-sm font-medium text-gray-700">Catégorie</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let item of filteredCategories()">
                <td class="px-6 py-4 text-sm text-gray-800">{{ item.createdAt | firestoreDate | date:'medium' }}</td>
                <td class="px-6 py-4 text-sm text-gray-800">{{ item.categorie }}</td>
                <td class="px-6 py-4 text-sm text-gray-600 max-w-[300px] truncate">
                  {{ item.description }}
                </td>

                <td class="px-6 py-4 text-sm text-gray-600 space-x-2  whitespace-nowrap">
                  <button class="btn-secondary" (click)="editCategory(item)">Modifier</button>
                  <button class="btn-secondary text-red-600 hover:text-red-800" (click)="deleteCategory(item)">Supprimer</button>
                </td>
              </tr>
    ...
    <div *ngIf="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6 animate-slide-in-right">
        ...
        <input type="text" [(ngModel)]="newCategory.name" class="form-input mb-4" />

        <label class="form-label">Description</label>
        <textarea [(ngModel)]="newCategory.description" rows="3" class="form-input mb-4"></textarea>

        <div class="flex justify-end space-x-2">
          <button class="btn-secondary" (click)="closeModal()">Annuler</button>
          <button class="btn-primary" (click)="saveCategory()">Enregistrer</button>
        </div>
      </div>
    </div>
  `
...
export class ChecksFiagnostique{
    ...
    isLoading = true; isEditMode = false;
    categorieId: string | null = null;

    private _garageId!: string;
    ...
    private async loadDateCategorie(): Promise<void>{
        this.isLoading = true
        try {
            this.categories = await this.garageDataService.getAll<DiagnosticCategory>('diagnosticCategory')
        }  catch (error) {
    ...
    filteredCategories() {
        ...
        c.categorie.toLowerCase().includes(term) || c.description.toLowerCase().includes(term)
        );
    }
           ...
           if(!this.newCategory)  throw new Error('No categorie is note null');
            ...
            if (this.newCategory.name.trim()) {
                ...
                if (this.isEditMode && this.categorieId) {
                    await this.garageDataService.update('diagnosticCategory', this.categorieId, checkDiagnostique);
                    this.notificationService.showSuccess('Diagnostic categorie updated successfully');
                } else {
                    await this.garageDataService.create('diagnosticCategory', checkDiagnostique);
                    this.notificationService.showSuccess('Diagnostic categorie report created successfully');
                }
                this.closeModal();
            } else {
             ...
             this.notificationService.showError('Failed to create diagnostic categorie report');
        }
