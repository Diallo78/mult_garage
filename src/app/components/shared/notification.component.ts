...
@Component({
  ...
  template: `
    <div
      class="fixed top-4 right-2 sm:right-4 z-50 space-y-2 w-full max-w-xs sm:max-w-sm"
    >
      <div
        *ngFor="let notification of notifications$ | async"
        class="bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 transform transition-all duration-300 ease-in-out animate-slide-in mx-2 sm:mx-0"
        [ngClass]="{
          'border-l-4 border-green-400': notification.type === 'success',
          'border-l-4 border-red-400': notification.type === 'error',
          'border-l-4 border-yellow-400': notification.type === 'warning',
      ...
      >
        <div class="p-3 sm:p-4">
          <div class="flex items-start">
            <div class="flex-shrink-0">
              <div
                class="h-5 w-5 sm:h-6 sm:w-6 rounded-full flex items-center justify-center text-white font-bold text-xs sm:text-sm"
                [ngClass]="{
            ...
            <div class="ml-2 sm:ml-3 w-0 flex-1 min-w-0">
              <p
                class="text-xs sm:text-sm font-medium text-gray-900 leading-tight"
              >
            ...
            <div class="ml-2 sm:ml-4 flex-shrink-0 flex">
              <button
                (click)="removeNotification(notification.id)"
                class="bg-white rounded-md inline-flex text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 p-1 touch-target"
              >
