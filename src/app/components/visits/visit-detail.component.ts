    ...
    <div *ngIf="!isLoading">
      <div class="space-y-6" *ngIf="visit">
        <div class="md:flex md:items-center md:justify-between">
          <div class="flex-1 min-w-0">
            ...
            <p class="text-lg text-gray-600">
              {{ visit.visitDate | firestoreDate | date : 'full' }}
            </p>
          </div>
        ...
        <div class="card" *ngIf="visit.driverId">
          <h3 class="text-lg font-medium text-gray-900 mb-4">
            Informations sur conducteur
          </h3>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label class="text-sm font-medium text-gray-500"
                >Nom du chauffeur</label
              >
              <p class="mt-1 text-sm text-gray-900">
                {{ visit.driverId.name }}
              </p>
        ...
        <div class="card">
          ...
          <div class="space-y-2">
            <div
              *ngFor="let issue of visit.reportedIssues; let i = index"
              class="flex items-start"
            >
              <span
                class="flex-shrink-0 w-6 h-6 bg-primary-100 text-primary-600 rounded-full flex items-center justify-center text-sm font-medium mr-3"
              >
                {{ i + 1 }}
              </span>
  ...
  private async loadVisitData(): Promise<void> {
    ...
    try {
      ...
      if (this.visit) {
        ...
        const diagnostics =
          await this.garageDataService.getWithFilter<Diagnostic>(
            'diagnostics',
