...
export class PersonnelDetailComponent implements OnInit {
  ...
  private async loadPersonnelData(): Promise<void> {
    try {
      this.personnel = await this.garageDataService.getById<Personnel>('personnel', this.personnelId!);
      if (this.personnel) {
  ...
  getRoleClass(role: string): string {
    switch (role) {
      case 'AdminGarage': return 'bg-red-100 text-red-800';
  ...
  async toggleStatus(): Promise<void> {
    ...
    try {
      ...
      await this.garageDataService.update('personnel', this.personnelId!, { isActive: newStatus });
      this.personnel.isActive = newStatus;
      this.notificationService.showSuccess(`Employee ${newStatus ? 'activated' : 'deactivated'} successfully`);
    } catch (error) {
