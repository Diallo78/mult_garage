...
@Component({
  ...
  `],
  template: `
  <div *ngIf="isLoading" class="flex flex-col justify-center items-center h-[60vh]">
    <div class="animate-spin rounded-full h-12 w-12 border-t-4 border-primary-500 border-solid mb-4"></div>
    ...
    <div class="max-w-4xl mx-auto p-4">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-primary-700 flex items-center">
          ...
          <span *ngIf="unreadCount > 0" class="ml-3 bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded-full">
      ...
      <div class="bg-white p-4 rounded-lg shadow-sm mb-4 border border-gray-200">
        ...
        <div class="flex flex-wrap gap-3 items-center">
          <div class="flex items-center">
            <label class="mr-2 text-sm font-medium text-gray-700">Filtrer par :</label>
            <select 
              [(ngModel)]="filterType" 
              (change)="applyFilters()"
          ...
          <div class="flex items-center ml-3">
            <label class="mr-2 text-sm font-medium text-gray-700">Période :</label>
            <select 
              [(ngModel)]="dateFilter" 
              (change)="applyFilters()"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 p-2"
          ...
          <div class="flex items-center">
            <label class="mr-2 text-sm font-medium text-gray-700">Trier par :</label>
            <select 
              [(ngModel)]="sortOrder" 
              (change)="applyFilters()"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-primary-500 focus:border-primary-500 p-2"
      ...
      <div *ngIf="notifications.length > 0" class="mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm">
        <div class="flex justify-between items-center">
          <div>
            <span class="font-medium">{{ filteredNotifications.length }}</span> notification{{ filteredNotifications.length !== 1 ? 's' : '' }} 
            <span class="text-gray-500">sur {{ notifications.length }} au total</span>
          </div>
          <button 
            *ngIf="filterType !== 'all' || dateFilter !== 'all' || sortOrder !== 'newest'"
            (click)="resetFilters()"
      ...
      <div class="bg-white shadow rounded-lg border border-gray-200 overflow-hidden transition-all duration-300">
        <div class="divide-y divide-gray-200 max-h-[60vh] overflow-y-auto">
          <div *ngFor="let notif of filteredNotifications; let i = index" 
                 ...
                 <div *ngIf="!notif.read" class="absolute top-0 right-0 bottom-0 left-0 animate-pulse-blue opacity-50 pointer-events-none rounded-md"></div>
            <div class="flex justify-between items-start">
              <div class="flex-1">
                <div class="flex items-center">
                  <div *ngIf="!notif.read" class="w-2 h-2 bg-blue-500 rounded-full mr-2"></div>
                  <h3 class="text-lg font-medium" [ngClass]="notif.read ? 'text-gray-700' : 'text-gray-900'">
                    {{ notif.title }}
                  </h3>
                </div>
                ...
                <div class="flex items-center mt-2 text-xs text-gray-500">
                  <span class="flex items-center">
                    ...
                    {{ notif.createdAt | firestoreDate | date:'dd/MM/yyyy à HH:mm' }}
                  </span>
                  <span *ngIf="notif.quoteId" class="ml-3 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div *ngIf="filteredNotifications.length === 0" class="text-center bg-white p-8 rounded-lg shadow-sm mt-4 border border-gray-200">
        ...
        <p class="text-gray-400 text-sm mt-1">Changez les filtres ou revenez plus tard</p>
      </div>

      <!-- Pagination -->
      <div *ngIf="notifications.length > 0" class="mt-4 flex justify-between items-center">
        <div class="text-sm text-gray-600">
          Affichage de {{ filteredNotifications.length }} notification{{ filteredNotifications.length > 1 ? 's' : '' }} sur {{ notifications.length }}
        </div>
      </div>
    </div>
  </div>
...
export class NotificationMessage implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      if (unreadCount > 0) {
        this.showSuccess(`Vous avez ${unreadCount} notification${unreadCount > 1 ? 's' : ''} non lue${unreadCount > 1 ? 's' : ''}`);
      }
  ...
  private async loadNotifications(): Promise<void> {
    ...
    this.lastNotificationCount = this.notifications.filter(n => !n.read).length;
  ...
  private startAutoRefresh(): void {
    ...
    this.refreshInterval = setInterval(async () => {
      ...
      const currentUnreadCount = this.notifications.filter(n => !n.read).length;
      if (currentUnreadCount > previousUnreadCount) {
        const newCount = currentUnreadCount - previousUnreadCount;
        this.showSuccess(`${newCount} nouvelle${newCount > 1 ? 's' : ''} notification${newCount > 1 ? 's' : ''}`);
      }
    }, 120000); // 2 minutes
  }
  ...
  applyFilters(): void {
    ...
    if (this.dateFilter !== 'all') {
      ...
      filtered = filtered.filter(notif => {
        const notifDate = notif.createdAt instanceof Date ? notif.createdAt : notif.createdAt.toDate();
  ...
  async markAllAsRead(): Promise<void> {
    try {
      ...
      this.showSuccess(`${unreadCount} notification${unreadCount > 1 ? 's' : ''} marquée${unreadCount > 1 ? 's' : ''} comme lue${unreadCount > 1 ? 's' : ''}`);
    } catch (error) {
