      ...
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="flex items-center">
              ...
              <div class="ml-4">
                ...
                <div class="text-2xl font-bold text-gray-900">
                  GNF {{ metrics.totalRevenue.toFixed(2) }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            <div class="flex items-center">
              <div class="flex-shrink-0">
                <div
                  class="flex items-center justify-center h-8 w-8 rounded-md bg-purple-500 text-white"
                >
                  🔧
                </div>
              </div>
          ...
          <div class="card">
            <div class="flex items-center">
              <div class="flex-shrink-0">
                <div
                  class="flex items-center justify-center h-8 w-8 rounded-md bg-orange-500 text-white"
                >
                  📊
                </div>
              </div>
              <div class="ml-4">
                ...
                <div class="text-2xl font-bold text-gray-900">
                  GNF {{ metrics.averageInvoice.toFixed(2) }}
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="card">
            ...
            <div class="space-y-3">
              <div
                *ngFor="let month of monthlyRevenue"
                ...
                <div class="flex items-center space-x-2">
                  <div class="w-32 bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      [style.width.%]="
                        (month.revenue / getMaxMonthlyRevenue()) * 100
                      "
                    ></div>
                  </div>
                  <span class="text-sm font-medium text-gray-900"
                    >GNF {{ month.revenue.toFixed(2) }}</span
                  >
                </div>
              </div>
            </div>
          ...
          <div class="card">
            ...
            <div class="space-y-3">
              <div
                *ngFor="let category of diagnosticCategories"
                class="flex items-center justify-between"
              >
                ...
                <div class="flex items-center space-x-2">
                  <div class="w-32 bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      [style.width.%]="
                        (category.count / getMaxCategoryCount()) * 100
                      "
                    ></div>
                  </div>
                  <span class="text-sm font-medium text-gray-900">{{
        ...
        <div class="card">
          <h3 class="text-lg font-medium text-gray-900 mb-4">
            Principaux clients par chiffre d'affaires
          </h3>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr *ngFor="let client of topClients">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    GNF {{ client.totalSpent.toFixed(2) }}
                  </td>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="text-center">
              <div class="text-3xl font-bold text-green-600">
                {{ serviceMetrics.averageCompletionTime.toFixed(1) }}h
              </div>
              <div class="text-sm text-gray-500">Temps d'exécution moyen</div>
            </div>
            <div class="text-center">
              <div class="text-3xl font-bold text-blue-600">
                {{ serviceMetrics.customerSatisfaction.toFixed(1) }}%
              </div>
              <div class="text-sm text-gray-500">Satisfaction client</div>
            </div>
            <div class="text-center">
              <div class="text-3xl font-bold text-purple-600">
                {{ serviceMetrics.repeatCustomers.toFixed(1) }}%
              </div>
              <div class="text-sm text-gray-500">Clients réguliers</div>
            </div>
          </div>
        </div>
      </div>
...
export class ReportDashboardComponent implements OnInit {
  ...
  private calculateMetrics(): void {
    ...
    // Basic metrics
    this.metrics.totalRevenue = filteredInvoices.reduce(
    ...
    this.metrics.averageInvoice =
      filteredInvoices.length > 0
        ? this.metrics.totalRevenue / filteredInvoices.length
        : 0;
    ...
    invoices.forEach((invoice) => {
      invoice.payments.forEach((payment) => {
        methods[payment.method] =
          (methods[payment.method] || 0) + payment.amount;
    ...
    this.visits.forEach((visit) => {
      ...
      if (client) {
        ...
        if (clientData[clientName]) {
          ...
          if (
            !clientData[clientName].lastVisit ||
            new Date(visit.visitDate) >
              new Date(clientData[clientName].lastVisit)
          ) {
  ...
  private calculateServiceMetrics(): void {
    ...
    const completedInterventions = this.interventions.filter(
      (i) => i.actualDuration
    );
    this.serviceMetrics.averageCompletionTime =
      completedInterventions.length > 0
        ? completedInterventions.reduce(
    ...
    this.visits.forEach((visit) => {
      clientVisitCounts[visit.clientId] =
        (clientVisitCounts[visit.clientId] || 0) + 1;
    });

    const repeatCustomers = Object.values(clientVisitCounts).filter(
      (count) => count > 1
    ).length;
    this.serviceMetrics.repeatCustomers =
      this.clients.length > 0
        ? (repeatCustomers / this.clients.length) * 100
        : 0;
  }
